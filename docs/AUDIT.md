...
- Run `session_today`. If open workout exists with `age_days ≥ 1` or `last_set_created` gap > 8h, and it appears in `session_calendar`/`session_stats`/`snapshot_export` counts.
...
- For each tracked muscle in `constants.json` (`constants_show`): compute weekly sets for each of the last 8 weeks (current week + 7 back) from `session_range`. Weeks with no logged sets count as 0, not as absent. Two separate flags, counted over the whole window (a good week in between does not reset the count):
