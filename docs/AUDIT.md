...
- For each tracked muscle in `constants.json` (`constants_show`): compute weekly sets for each of the last <!--const thresholds.volume_window_weeks-->8<!--/const--> weeks (current week + 7 back) from `session_range`. Weeks before the first logged sets for that muscle are pre-history, not absences; weeks with no logged sets after that count as 0. A muscle never trained keeps the whole window and flags. Two separate flags, counted over the trained span (a good week in between does not reset the count):
