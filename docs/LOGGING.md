...
11. On `done`, `finished`, or clear end of session, first write per-lift progression (`progression_set` for every trained lift with verdict, next target as weight x reps, direction; reps are required, bare weights are refused, e.g. `82.5x5` not `82.5`), optionally run `session_check` to preview the close gate, then run `session_end` with a short session summary (feel, sleep, pain, what moved well). `session_end` refuses to close while anything is outstanding and prints the exact fix per item; `session_end` with a force reason skips writeback items (progression, reconcile, deload note) with the reason recorded in the workout note, but never skips missing muscles. Then run `sync_push` to push the dashboard, then commit `workouts.sql` and push the repo (`data: <today's date>` as message). The sql dump is tracked in git, that commit is the backup and the undo button. This repo is fully agent written, committing and pushing here needs no permission.
