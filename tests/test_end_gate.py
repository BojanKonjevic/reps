...
def test_progression_next_requires_reps(log_module):
    log = log_module
    _seed_session(log)
    with pytest.raises(RepsError, match="weight x reps"):
        log.set_progression("bench", "hit", "102.5", "up")
...
def test_end_succeeds_after_progression(log_module):
    ...
    log.set_progression("squat", "hold", "150x5", "flat")
    out, code = _run(log.check_end_gate)
    assert code is True
    assert out["ready"]
...
def test_end_requires_deload_note(log_module):
    ...
    log.set_progression("bench", "hold", "100x5", "flat")
    log.set_progression("squat", "hold", "150x5", "flat")
...
def test_progression_rejects_unknown_exercise(log_module):
    ...
    with pytest.raises(RepsError, match="no sets in workout"):
        log.set_progression("deadlift", "hit", "180x5", "up")
...
def test_read_commands_roundtrip(log_module):
    ...
    _seed_session(log)
    log.set_progression("bench", "hit", "102.5x5", "up")
    assert log.get_progression("bench")[0]["next_target"] == "102.5x5"
...
def test_restore_accepts_new_tables(log_module, tmp_path, monkeypatch):
    ...
    log = log_module
    _seed_session(log)
    log.set_progression("bench", "baseline", "100x5", "flat")
