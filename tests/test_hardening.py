...
def test_log_warns_on_magnitude_jump_but_logs(log_module):
    ...
    out = log_module.log_set("bench", 200, 5, "", "chest")
...
def test_log_warns_on_same_workout_collapse(log_module):
    ...
    log_module.start_workout("test")
    log_module.log_set("bench", 100, 5, "", "chest")
    out = log_module.log_set("bench", 10, 5, "", "chest")
    assert "warnings" in out
...
def test_log_warns_on_near_duplicate_name(log_module):
    ...
    c = log_module.conn()
    log_module.start_workout("test")
    log_module.log_set("flat barbell bench press", 90, 5, "", "chest")
    out = log_module.log_set("flat barbell bench pres", 90, 5, "", "chest")
    assert "warnings" in out
...
def test_log_warns_on_stale_open_workout(log_module):
    ...
    c.commit()
    out = log_module.log_set("bench", 100, 5, "", "chest")
    assert "warnings" in out
...
def test_update_warns_on_magnitude(log_module):
    ...
    log_module.start_workout("test")
    log_module.log_set("bench", 100, 5, "", "chest")
    log_module.log_set("bench", 90, 5, "", "chest")
    sets = c.execute("SELECT id FROM sets ORDER BY id").fetchall()
    out = log_module.update_set(sets[1]["id"], "weight", "200")
    assert "warnings" in out
...
def test_update_workout_rejects_second_open(log_module):
    ...
    c = log_module.conn()
    log_module.start_workout("first")
...
def test_rename_refuses_conflicting_mapping(log_module):
    ...
    c = log_module.conn()
    log_module.start_workout("test")
    log_module.log_set("bp", 50, 8, "", "chest,triceps")
    log_module.log_set("press", 60, 8, "", "chest,front delts")
    close_session(log_module, "done")
    try:
...
def test_rename_same_mapping_merges(log_module):
    ...
    c = log_module.conn()
    log_module.start_workout("test")
    log_module.log_set("bp", 50, 8, "", "chest")
    log_module.log_set("bench", 100, 5, "", "chest")
    close_session(log_module, "done")
    out = log_module.rename_exercise("bp", "bench")
...
def test_bad_numeric_inputs_exit_cleanly(log_module):
    ...
    wid = c.execute("SELECT id FROM workouts").fetchone()["id"]
    cases = [
        (log_module.log_set, ("bench", "abc", 5, "", "chest"), "must be a number"),
        (log_module.record_bodyweight, ("abc", ""), "must be a number"),
        (log_module.get_history, ("bench", "abc"), "integer"),
        (log_module.get_session, ("not-a-date",), "YYYY-MM-DD"),
        (log_module.get_session_range, ("not-a-date", "2026-01-01"), "YYYY-MM-DD"),
        (log_module.update_set, (set_id, "weight", "abc"), "must be a number"),
        (log_module.update_set, (set_id, "reps", "abc"), "integer"),
...
def test_rename_same_muscles_different_order_merges(log_module):
    ...
    c = log_module.conn()
    log_module.start_workout("test")
    log_module.log_set("bp", 50, 8, "", "triceps,chest")
    log_module.log_set("bench", 60, 8, "", "chest,triceps")
    close_session(log_module, "done")
    out = log_module.rename_exercise("bp", "bench")
    assert out["renamed"] == 1
    assert out["map_moved"] is True
