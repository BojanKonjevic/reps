...
def test_rest_creates_rest_row(log_module):
    ...
    c = log_module.conn()
    out = json.loads(capture_stdout(log_module.rest, date.today().isoformat(), "sore"))
...
def test_rest_repeat_without_note_reports_noop(log_module):
    ...
    log_module.rest(today, "sore")
    out = json.loads(capture_stdout(log_module.rest, today, ""))
    assert out["appended"] is False
...
def test_rest_rejects_bad_date_cleanly(log_module):
    ...
    try:
        log_module.rest("not-a-date", "sore")
...
def test_rest_second_call_appends_note(log_module):
    ...
    c = log_module.conn()
    today = date.today().isoformat()
    first = json.loads(capture_stdout(log_module.rest, today, "sore"))
    second = json.loads(capture_stdout(log_module.rest, today, "tired"))
...
def test_rest_refused_with_open_workout(log_module):
    ...
    try:
        log_module.rest(date.today().isoformat(), "tired")
        assert False, "should have exited"
    except SystemExit as e:
...
def test_rest_refused_for_future_date(log_module):
    ...
    try:
        log_module.rest(future, "planned")
        assert False, "should have exited"
    except SystemExit as e:
...
def test_rest_accepts_past_date(log_module):
    ...
    out = json.loads(capture_stdout(log_module.rest, past, "travel"))
...
def test_start_still_works_on_rest_day(log_module):
    ...
    c = log_module.conn()
    log_module.rest(date.today().isoformat(), "was going to rest")
    out = json.loads(capture_stdout(log_module.start, "changed mind"))
...
def test_stats_and_context_exclude_rest(log_module):
    ...
    log_module.rest(date.today().isoformat(), "sore")
    log_module.start("test")
    log_module.log("bench", 100, 5, "", "chest")
    close_session(log_module, "done")
    stats = json.loads(capture_stdout(log_module.stats))
    assert stats["workouts"] == 1
    ctx = json.loads(capture_stdout(log_module.context, "5"))
...
def test_calendar_marks_rest_days(log_module):
    ...
    c = log_module.conn()
    log_module.rest(date.today().isoformat(), "sore")
    out = json.loads(capture_stdout(log_module.calendar))
...
def test_today_reports_rest_entry(log_module):
    ...
    log_module.rest(date.today().isoformat(), "sore legs")
    out = json.loads(capture_stdout(log_module.today))
