...
def test_goal_add_builds_linear_trajectory(log_module):
    log = log_module
    _seeded(log)
    out = json.loads(_out(log.cmd_goal_add, "bench", 130, _deadline(), "test goal", None))
    assert out["sessions"] >= 8
    rows = json.loads(_out(log.cmd_goal_show, 1))
    assert len(rows) == 1
...
def test_goal_add_rejects_past_deadline_and_unknown_lift(log_module):
    ...
    with pytest.raises(SystemExit, match="future"):
        log.cmd_goal_add("bench", 130, "2020-01-01", "", None)
    with pytest.raises(SystemExit, match="no mapping"):
        log.cmd_goal_add("mystery press", 130, _deadline(), "", 100)


def test_goal_rewrite_anchors_on_latest_actual(log_module):
    log = log_module
    _seeded(log, days_ago=6)
    log.cmd_goal_add("bench", 130, _deadline(), "", None)
    before = json.loads(_out(log.cmd_goal_show, 1))[0]["checkpoints"]
    log.cmd_start("session 2")
    log.cmd_log("bench", 110, 5, "", "chest")
    close_session(log, "big jump")
    out = json.loads(_out(log.cmd_goal_rewrite, 1))
    assert out["rewritten_from_session"] == 3
    after = json.loads(_out(log.cmd_goal_show, 1))[0]["checkpoints"]
...
def test_goal_divergence_flags_in_audit(log_module):
    ...
    log.cmd_goal_add("bench", 200, _deadline(), "", None)
    c0 = log.conn()
...
def test_goal_slippage_when_frequency_too_low(log_module):
    log = log_module
    _seeded(log)
    log.cmd_goal_add("bench", 130, _deadline(60), "", None)
    c = log.conn()
...
def test_goal_drop(log_module):
    ...
    log.cmd_goal_add("bench", 130, _deadline(), "", None)
    log.cmd_goal_drop(1)
    assert json.loads(_out(log.cmd_goal_show, None)) == []
