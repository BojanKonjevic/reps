...
async function main() {
  ...
  bwCv.addEventListener("mousemove", (ev) => {
    ...
    showTip(
      BWDATA[idx].date,
      [[null, BWDATA[idx].kg.toFixed(1) + " kg"]],
      ev.clientX,
      ev.clientY,
    );
  });
...
function render() {
  ...
  for (const s of S) {
    if (s.note && s.note.toLowerCase().includes("grindy")) {
      const d = wday(s);
      noted[d] =
        (noted[d] ? noted[d] + " / " : "") + s.exercise + ": " + s.note;
    }
  }
  ...
  for (const s of S) {
    const d = wday(s);
    (dayDetail[d] = dayDetail[d] || []).push(
      s.exercise + " " + s.weight + "x" + s.reps,
    );
  }
  ...
  Object.keys(prs)
    ...
    .forEach((k) => {
      ...
      b2.textContent =
        p.s.weight + " x " + p.s.reps + " (e1RM " + p.ev.toFixed(1) + ")";
  ...
  for (let d = 1; d <= days; d += 1) {
    ...
    if (trained) {
      ...
      el.addEventListener("mousemove", (ev) => {
        ...
        if (order.length > 6)
          rows.push([null, "+" + (order.length - 6) + " more lifts"]);
        const wnotes = (wByDate[key] || [])
          .map((w) => w.notes)
          .filter((n) => n);
        ...
        showTip(
          title,
          rows.length ? rows : [[null, "tap to open"]],
          ev.clientX,
          ev.clientY,
        );
      });
